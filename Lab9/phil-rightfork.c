...
void *phil_thread(void *arg) {
  ...
  while (meals < num_meals) {
    ...
    printf("philosopher %ld got the right fork.\n",id);
    nap();
    getfork(leftfork(id));
    printf ("philosopher %ld got the left fork.\n",id);
    nap();
    printf ("philosopher %ld eats the spaghetti.\n",id);
    printf ("philosopher %ld puts down the forks.\n",id);
    putfork(id);
    nap();
    num_meals--;
  }
