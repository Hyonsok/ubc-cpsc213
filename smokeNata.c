...
void* agent (void* av) {
  ...
  // todo--- choices: array with possible combinations
  // todo--- matching_smoker: array of opposite resource; binded by index
  srandom(time(NULL));
  ...
  for (int i = 0; i < NUM_ITERATIONS; i++) {
    int r = random() % 3;
    signal_count [matching_smoker [r]] ++; // signal count at either: MATCH|PAPER, MATCH|TOBACCO, PAPER|TOBACCO is incremented
    int c = choices [r]; // if r = 0, c = MATCH|PAPER. lets say r = 0, then 2 of the following statements are possible
    if (c & MATCH) { //
  ...
  uthread_mutex_unlock (a->mutex); // once the agent is done signalling, unlock mx
  return NULL;
}
